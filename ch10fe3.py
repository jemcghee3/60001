...
class Politician(Person):
    ...
    def might_agree(self, other):
        ...
        party"""
        if self.get_party() == None or other.get_party() == None:
            return True
        if self.get_party() == other.get_party():
            return True
        return False # Instructions do not say to return false, so I could have not.

a = Politician('Adam', 'American Party')
b = Politician('Joe', 'American Party')
c = Politician('Hans', 'Swiss Party')
d = Politician('Adam', 'Swiss Party')
...
for n in [a, b, c, d]:
    for m in [a, b, c, d]:
        if n == m:
            continue
        print(f'{n.get_name()} might agree with {m.get_name()} is {n.might_agree(m)}.')
        print(f'{n.get_name()} is a member of the {n.get_party()} and {m.get_name()} is in the {m.get_party()}.')
